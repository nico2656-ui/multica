...
function MemberRow({
  ...
  onRemove: () => void;
  roleConfig: Record<MemberRole, { label: string; icon: typeof Crown; description: string }>;
}) {
    ...
    <div className="flex items-center gap-3 px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-1 text-xs text-muted-foreground">
          ...
          <span>{t.settings.pendingInvitations_}</span>
        </div>
      </div>
      {canManage && (
        <Button
          ...
          disabled={busy}
          onClick={onRevoke}
          title={t.settings.revocationInvitation}
        >
...
export function MembersTab() {
  ...
  const roleConfig: Record<MemberRole, { label: string; icon: typeof Crown; description: string }> = {
    owner: { label: t.settings.owner, icon: Crown, description: t.settings.roleOwnerDesc },
    admin: { label: t.settings.admin, icon: Shield, description: t.settings.roleAdminDesc },
    member: { label: t.settings.member, icon: User, description: t.settings.roleMemberDesc },
  };
  ...
  const handleInviteMember = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.settings.invitationSendFailed);
    } finally {
  ...
  const handleRevokeInvitation = (invitation: Invitation) => {
    ...
    setConfirmAction({
      title: t.settings.revocationInvitation,
      description: t.settings.revocationInvitationDesc(invitation.invitee_email),
      variant: "destructive",
      onConfirm: async () => {
  ...
  const handleRemoveMember = (member: MemberWithUser) => {
    ...
    setConfirmAction({
      ...
      description: t.settings.removeMemberDesc(member.name, workspace.name),
      variant: "destructive",
      onConfirm: async () => {
